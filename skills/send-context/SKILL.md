...
The skill takes an optional argument: the destination folder where the handoff file should be written.
...
- Date and time the handoff was generated (use ISO 8601 in the user's local time).
- The originating working directory (absolute path).
- The originating git repo, branch, and short commit SHA if applicable. Run `git rev-parse --show-toplevel`, `git branch --show-current`, and `git rev-parse --short HEAD` to gather these. Skip silently if not in a git repo.
- A one-sentence "subject line" describing what this handoff is about.
...
4. Draft the seven sections from conversation context. If a section would be empty or trivial, omit it (except the header and goal, which are required).
5. Write the file with the `Write` tool.
6. Report to the user: the absolute path of the file, its line count, and a one-line confirmation of what was captured. Do not paste the file's contents back into the conversation.
